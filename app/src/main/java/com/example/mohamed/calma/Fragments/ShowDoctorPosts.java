...
public class ShowDoctorPosts extends Fragment {
    ...
    public static ShowDoctorPosts newPosts(User type){
        mUser=type;
        return new ShowDoctorPosts();
    }
    @Nullable
    @Override
    ...
    private void UpdateUi(List<UserPost> posts,String postId){
     PostsAdapter adapter=new PostsAdapter(posts,postId);
        mRecyclerView.setAdapter(adapter);
        adapter.notifyDataSetChanged();
    }

    private void getPosts(){
        final DatabaseReference mReference=mDatabaseReference.child("doctor");
                        ...
                        for (final DataSnapshot snapshot1 : snapshot.getChildren()) {
                            ...
                            posts.addValueEventListener(new ValueEventListener() {
                                ...
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    ...
                                    //         Toast.makeText(getActivity(), snapshot.getKey() + "/" + snapshot1.getKey(), Toast.LENGTH_SHORT).show();
